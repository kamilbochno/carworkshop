...
function AddCarShopItem() {
  const { isOpenAddCarShopItem, setIsOpenAddCarShopItem, itemCategory, getCarShopItems } =
  ...
  const handleSubmitCarShopItem: SubmitHandler<IFormInput> = async (data) => {
    ...
    try {
      await axios
        .post("/dashboard/admin/warehouse/carshopitems/add", formData, config)
        .then((res) => {
          if (res.status === 201) {
            setIsOpenAddCarShopItem(false);
            getCarShopItems();
      ...
      <div className="relative w-auto my-6 mx-auto max-w-3xl">
        <div className="border-2 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
          <div className="flex items-center justify-between p-6 border-b border-solid border-gray-300 rounded-t ">
            <h3 className="text-2xl font=semibold mx-auto">Add new car shop item</h3>
            <button
              type="button"
              onClick={() => setIsOpenAddCarShopItem(false)}
          ...
          <div className="relative p-4 flex-auto">
            <form className="px-8 w-full" onSubmit={handleSubmit(handleSubmitCarShopItem)}>
