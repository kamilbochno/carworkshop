...
function CarRepairShopItemEdit() {
  const {
  ...
  useEffect(() => {
    if (carRepairShopItem) {
      reset({
        title: carRepairShopItem.title,
        description: carRepairShopItem.description,
        price: carRepairShopItem.price,
        quantity: carRepairShopItem.quantity,
        category: carRepairShopItem.category
      });
    }
  }, [carRepairShopItem]);

  const handleSubmitCarRepairShopItemEdit: SubmitHandler<IFormInput> = async (data) => {
    data.key = carRepairShopItem.key;
    data.itemId = carRepairShopItem._id;
    ...
    try {
      await axios
        .post("/dashboard/admin/warehouse/carrepairshopitems/edit", formData, config)
        .then((res) => {
          if (res.status === 201) {
            setIsOpenCarRepairShopItemEdit(false);
            getCarRepairShopItems();
  ...
  if (!isOpenCarRepairShopItemEdit) return null;
        ...
        <div className="relative w-auto my-6 mx-auto max-w-3xl">
          <div className="border-2 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            <div className="flex items-center justify-between p-6 border-b border-solid border-gray-300 rounded-t ">
              <h3 className="text-2xl font=semibold mx-auto">Edit car repair shop item</h3>
              <button
                type="button"
                onClick={() => setIsOpenCarRepairShopItemEdit(false)}
            ...
            <div className="relative p-4 flex-auto">
              <form
                ...
                onSubmit={handleSubmit(handleSubmitCarRepairShopItemEdit)}>
...
export default CarRepairShopItemEdit;
